...
export default class Home extends React.Component {
	...
	fetchProductStatusActions(productDetails) {
		...
		if (this.props.userType === USER_TYPES[1]) {
			if (productDetails['productStatus'] === 2) {
				...
				productDetails['productStatus'] === 6 &&
		...
		if (this.props.userType === USER_TYPES[2]) {
			if (
				productDetails['productStatus'] === 4 &&
				productDetails['retailerAddresses'] !== this.state.addressZero
			) {
				statusAction = STATUS_ACTIONS[5];
			} else if (
	...
	fetchActiveBatches(rows) {
		...
		if (this.props.userType === USER_TYPES[0]) {
			return rows
				.filter((row) => row.productStatus !== PRODUCT_STATUSES[7])
				.reverse();
		}
	...
	checkProductDetails(contractName) {
		...
		const product = productDetailsArray.value.filter(
			(productDetails) =>
	...
	render() {
		if (this.props.isAuthenticated || this.props.isSuccessfullyRegistered) {
			...
			if (this.props.newUserType === USER_TYPES[3]) {
				return (
					...
					>
						...
						<TabPanel value={this.state.tabValue} index={0} count={2}>
							{this.props.userType === USER_TYPES[0] ? (
								<div>
									...
									>
										<AddIcon
											style={{ paddingRight: '4px', paddingTop: '1px' }}
										/>
										Produce a New Batch
									</Button>
									<br />
							...
							<BatchTable
								...
								toggleBatchDetailsPopUp={(prodRow) =>
									this.toggleBatchDetailsPopUp(prodRow)
								}
								showConfirmActionPopUp={(action, id) =>
									this.showConfirmActionPopUp(action, id)
								}
						...
						{this.state.showAddBatch ? (
							<ErrorBoundary
								...
								setTransactionSuccess={(status) =>
									this.setTransactionSuccess(status)
								}
							>
								<ProductBatchForm
									...
									contractName={
										this.props.drizzle.contracts.SupplyChainLifecycle
									}
						...
						<CircularPageLoader open={this.state.showLoader} />
					</Paper>
				</div>
			);
		} else {
